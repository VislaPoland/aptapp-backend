...
public class PropertyController {
    ...
    public PageableWithTotalCountDataResponse<List<PropertyDto>> getAllProperties(@RequestParam(value="page",required=false) Integer page, 
    	...
    	ret = new PageableWithTotalCountDataResponse<List<PropertyDto>>(propertyService.getAllProperties(page, size, keywords).stream()
                .map(p -> mapper.toPropertyDto(p)).collect(Collectors.toList()),size,page,(propertyService.getAllPropertiesCount(keywords)).intValue(),keywords);
    	return ret;
    }
    ...
    @RoleSecured({AccountRole.Administrator, AccountRole.PropertyOwner, AccountRole.PropertyManager, AccountRole.AssistantPropertyManager, AccountRole.Security, AccountRole.Maintenance, AccountRole.Tenant, AccountRole.SubTenant})
